...
def get_contiguous_indices(indices_opt_init):
    ...
    where_change_contiguous_regions = np.where(running_diffs != 1)[
        0
    ]  # find the last contiguous index - 1

    if len(where_change_contiguous_regions) > 0:
        last_el_first_contiguous_region = where_change_contiguous_regions[0]
    elif (
        np.sum(running_diffs != np.ones(len(running_diffs))) == 0
    ):  # if all are optimal (mnist_2_6: coord=72)
        last_el_first_contiguous_region = len(running_diffs)
    elif (
        len(indices_opt_init) == 1
    ):  # the easiest and most common situation - just 1 optimal index
...
def finalize_curr_row(latex_str, weak_learner, flag_n_trees_latex):
    ...
    for i_val, val_str in enumerate(metrics_str.split(" & ")):
        ...
        val = (
            int(val_str)
            if flag_n_trees_latex and i_val % n_metrics == n_metrics - 1
            else float(val_str)
        )
    ...
    for tup in zip(*metrics_curr_row.values()):
        for i_m, m in enumerate(tup):
            ...
            if (
                m == min(metrics_curr_row[i_m])
                and not (flag_n_trees_latex and i_m == 3)
                and not (weak_learner == "stump" and i_m == 2)
            ):  # if URTE for stumps, don't boldface
        ...
        for model in models:
            ...
            if model_names_curr != []:
                ...
                for model_name_final in model_names_curr:
                    model_name_final = model_name_final.split(".metrics")[0].split(
                        exp_folder + "/"
                    )[1]
