...
def ijcnn1():
    ...
    for i, line in enumerate(open(path_train, "r").readlines()):
        ...
        for s in line.split(" ")[1:]:
            ...
            coord, val = (
                int(coord_str) - 1,
                float(val_str),
            )  # -1 is needed to have pythonic numeration from 0
            X_train[i, coord] = val
        num_train_orig += 1
    ...
    assert (
        num_train == num_train_orig + num_val_orig
    )  # Check that we have not introduced extra zero rows
...
def mnist_1_5():
    ...
    X_train, X_test = (
        ...
        X_test.astype(np.float64) / 255.0,
    )
...
def har():
    ...
    X_train, X_test = (
        np.loadtxt(path_train + "X_train.txt"),
        np.loadtxt(path_test + "X_test.txt"),
    )
    y_train, y_test = (
        np.loadtxt(path_train + "y_train.txt"),
        np.loadtxt(path_test + "y_test.txt"),
    )
...
def data_augment(X, dataset):
    ...
    for i in range(num):
        X_img[i] = random_crop(
            X_img_pad[i], n_crop=n_crop
        )  # up to `n_crop` pixels are cropped
