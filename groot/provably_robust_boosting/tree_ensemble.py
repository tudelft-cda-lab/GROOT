...
def find_min_yf_point(nodes, x, y, eps):
    ...
    while len(node_ids_to_explore) > 0:
        ...
        id_left, id_right, w_l, w_r, b, coord = (
            int(node[1]),
            int(node[2]),
            node[3],
            node[4],
...
class Tree:
    def __init__(
        self, id_=-1, left=None, right=None, w_l=0.0, w_r=0.0, b=0.0, coord=0, loss=0.0
    ):
    ...
    def __repr__(self):
        ...
        if self.left is None and self.right is None:
            return "if x[{}] < {:.4f}: {:.4f} else {:.4f}    ".format(
                self.coord, threshold, lval, rval
            )
        if self.left is None:
            return (
                "if x[{}] < {:.4f}: {:.4f}  ".format(self.coord, threshold, lval)
                + self.right.__repr__()
            )
        if self.right is None:
            return self.left.__repr__() + "if x[{}] >= {:.4f}: {:.4f}  ".format(
                self.coord, threshold, rval
            )

        s = ""
        if self.left is not None:
            s += (
                "if x[{}] < {:.4f} and ".format(self.coord, threshold)
                + self.left.__repr__()
            )
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, Tree):
            return (
                ...
                and self.w_l == other.w_l
                and self.w_r == other.w_r
                and self.b == other.b
    ...
    def predict(self, X):
        parallel = False
        if (
            parallel and len(self.node_list) > 0
        ):  # 2nd condition is needed to prevent an error in predict_tree_par()
    ...
    def predict_native(self, X):
        def predict_recursive(curr_tree, idx):
            ...
            # idx_left_superset = X[:, curr_tree.coord] <= round(curr_tree.b, 5)  # TODO: I added this!!
    ...
    def find_min_yf_native(self, X, y, eps):
        ...
        if self.left is None:
            ...
            left_min_yf = self.left.find_min_yf(
                X[guaranteed_left], y[guaranteed_left], eps
            )
        ...
        if self.right is None:
            ...
            right_min_yf = self.right.find_min_yf(
                X[guaranteed_right], y[guaranteed_right], eps
            )
    ...
    def get_n_nodes(self):
        ...
        subtree_n = (
            left_n + right_n
        )  # n nodes of the subtree rooted at the current node
    ...
    def get_depth(self):
        ...
        )  # depth of the subtree rooted at the current node
        return (
            subtree_depth + 1
        )  # which means that a decision stump is a tree of depth=1
    ...
    def get_json_dict(self, counter_terminal_nodes):
        ...
        if self.left is None:
            ...
            children_list.append(
                {"nodeid": id_left, "leaf": round(self.w_l, precision)}
            )  # end node
        else:
            id_left = self.left.id
            children, counter_terminal_nodes = self.left.get_json_dict(
                counter_terminal_nodes
            )
        ...
        if self.right is None:
            ...
            counter_terminal_nodes -= 1
            children_list.append(
                {"nodeid": id_right, "leaf": round(self.w_l + self.w_r, precision)}
            )  # end node
        else:
            id_right = self.right.id
            children, counter_terminal_nodes = self.right.get_json_dict(
                counter_terminal_nodes
            )
            children_list.append(children)
...
class TreeEnsemble:
    ...
    def load(self, ensemble_dict, iteration):
        tree_indices = np.sort(
            list(ensemble_dict.keys())
        )  # just a list of contiguous indices [0, 1, ..., n_trees]
        ...
        for i_tree in tree_indices:
            ...
            for i_node in range(len(ensemble_dict[i_tree])):
                if not np.all(ensemble_dict[i_tree][i_node] == 0):
                    id_, id_left, id_right, w_l, w_r, b, coord, loss = ensemble_dict[
                        i_tree
                    ][i_node]
                    id_, id_left, id_right, coord = (
                        int(id_),
                        int(id_left),
                        int(id_right),
                        int(coord),
    ...
    def export_model(self):
        ...
        for i, tree in enumerate(self.trees):
            ensemble_dict[i] = np.array(
                tree.node_list
            )  # all tree nodes are in this array
    ...
    def prune_last_tree(self, X, y, margin_prev, eps, model):
        ...
        best_tree = copy.deepcopy(
            self.trees[-1]
        )  # copy the whole tree since we will change its leaves
        ...
        best_loss += (
            self.gamma_hp * best_tree.get_depth()
        )  # introduce depth penalization
        ...
        while best_loss >= loss_prev_ensemble and not (
            curr_tree.left is None and curr_tree.right is None
        ):
            ...
            if model in ["plain", "da_uniform", "at_cube"]:
                ...
                raise ValueError("wrong model type")
            loss_pruned += (
                self.gamma_hp * curr_tree.get_depth()
            )  # introduce depth penalization
    ...
    def fit_tree(self, X, y, gamma, model, eps, depth):
        ...
        """
        parallel = False  # True  # causes a minor memory leak; disable if the memory is limited

        if depth == 1:
            self.max_tree_node_id = 0  # if we start a new tree, set the counter to 0 (needed for efficient predict())
        if (
            depth > self.max_depth
        ):  # and (X.shape[0] <= 10000 or depth > 2*self.max_depth):  # adaptive depth
            return None
        ...
        w_l, w_r, b, coord, loss = self.fit_stumps_over_coords(
            X, y, gamma, model, eps, depth
        )
        ...
        if loss >= np.mean(
            gamma
        ):  # if the stump doesn't help, don't add it at all; very unlikely situation
        ...
        if parallel and depth <= 4:
            ...
            tree.left = self.fit_tree(
                X[idx_left, :], y[idx_left], gamma[idx_left], model, eps, depth + 1
            )
            # print("right subtree: {:d} examples".format(np.sum(idx_right)))
            tree.right = self.fit_tree(
                X[idx_right, :], y[idx_right], gamma[idx_right], model, eps, depth + 1
            )
    ...
    def fit_stumps_over_coords(self, X, y, gamma, model, eps, depth):
        ...
        if verbose:
            print(
                "[{}-vs-all] depth {}: n_ex {}, n_coords {} -- loss {:.5f}->{:.5f}, b={:.3f} wl={:.3f} wr={:.3f} at coord {}".format(
                    self.idx_clsf,
                    depth,
                    n_ex,
                    n_coords,
...
def fit_stump_batch(
    Xs, y, gamma, model, eps, coords, n_bins, min_samples_leaf, max_weight
):
    res = np.zeros([len(coords), 5])
    for i, coord in enumerate(coords):
        res[i] = fit_stump(
            Xs[:, i], y, gamma, model, eps, coord, n_bins, min_samples_leaf, max_weight
        )
    return res


def fit_stump(
    X_proj, y, gamma, model, eps, coord, n_bins, min_samples_leaf, max_weight
):
    min_prec_val = 1e-7
    min_val, max_val = (
        0.0,
    ...
    if model in ["plain", "da_uniform", "at_cube"]:
        losses, w_l_vals, w_r_vals, b_vals = fit_plain_stumps(
            X_proj, y, gamma, b_vals, max_weight
        )
    elif model == "robust_bound":
        losses, w_l_vals, w_r_vals, b_vals = fit_robust_bound_stumps(
            X_proj, y, gamma, b_vals, eps, max_weight
        )
    else:
    ...
    if (
        n_bins > 0
    ):  # note that one shouldn't average thresholds since it's unpredictable what is in between
    ...
    if model in ["plain", "da_uniform", "at_cube"]:
        ...
        loss_prev_half = exp_loss_robust(
            X_proj, y, gamma, w_l_prev, w_r_prev, [], [], b_prev_half, eps, False
        )

        b_next_half = (b_vals[indices_opt[-1]] + b_next) / 2
        loss_next_half = exp_loss_robust(
            X_proj, y, gamma, w_l_next, w_r_next, [], [], b_next_half, eps, False
        )
    ...
    if model in ["plain", "da_uniform", "at_cube"]:
        loss, w_l_opt, w_r_opt, _ = fit_plain_stumps(
            X_proj, y, gamma, b_val_final, max_weight
        )
    elif model == "robust_bound":
        loss, w_l_opt, w_r_opt, _ = fit_robust_bound_stumps(
            X_proj, y, gamma, b_val_final, eps, max_weight
        )
    else:
        raise ValueError("wrong model")
